...
$(document).ready(function () {
    ...
    function init_template() {
        ...
        */
        ...
        function showFixedAd() {
            setTimeout(function () {
                ad_fixed.fadeIn(350);
            }, 3500);
        }
        $('.close-ad-button').on('click', function () {
            ad_fixed.fadeOut(250);
        });
        if (ad_fixed.length) {
            showFixedAd();
        }
        ...
        function resize_coverpage() {
            ...
            if (cover_width.length > 0) {
                var header_height = "0";
                page_transitions.css({"min-height": cover_height});
            } else {
                var header_height = "0";
                page_transitions.css({"min-height": cover_height});
            }
        ...
        $('.read-more-show').on('click', function () {
            $(this).hide();
            $(this).parent().find('.product-content').css('max-height', '100%');
            $(this).parent().find('.product-content').css('height', 'auto');
            $('.read-less-show').show();
        });
        $('.read-less-show').on('click', function () {
            $(this).hide();
            $(this).parent().find('.product-content').css('height', '50px');
            $('.read-more-show').show();
        });
...
$('.video-detail-slider').owlCarousel({
    loop: true,
    margin: 15,
    nav: false,
    dot: true,
...
$('#password1').change(function () {
    if ($(this).val().length < 6) {
        var span = "<span class='help-block'>Nhập mật khẩu tối thiểu 6 kí tự</span>";
...
$('#password2').change(function (e) {
    if ($(this).val() != $('#password1').val()) {
        var span = "<span class='help-block'>Nhập lại mật khẩu không trùng khớp</span>";
        $(this).parent().append(span);
    } else {
        $('.help-block').remove();
    }
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (resp.success == false) {
                    var span = "<span class='help-block'>Username đã tồn tại</span>";
                    $this.parent().append(span);
                } else {
                    $('.help-block').remove();
                }
...
$(function () {
    $('.account').click(function () {
        $('.dropdown-menu').addClass('dropdown-menu-open');
    })
